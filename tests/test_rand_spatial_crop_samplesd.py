...
class TestRandSpatialCropSamplesd(unittest.TestCase):
    ...
    def test_value(self, input_param, input_data, expected_shape, expected_last):
        ...
        for i, item in enumerate(result):
            ...
            self.assertEqual(type(item["img"]), type(input_data["img"]))
            if isinstance(item["img"], torch.Tensor):
                self.assertEqual(item["img"].device, input_data["img"].device)
                item["img"] = item["img"].cpu()
                item["seg"] = item["seg"].cpu()
        np.testing.assert_allclose(item["img"], expected_last["img"])
        np.testing.assert_allclose(item["seg"], expected_last["seg"])

    def test_deep_copy(self):
        for p in TEST_NDARRAYS:
            data = {"img": p(np.ones((1, 10, 11, 12)))}
            num_samples = 3
