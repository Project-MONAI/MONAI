...
# test dataloader refers to `@dataset`, here we don't test recursive reference, test that in `ConfigResolver`
TEST_CASE_11 = [
    "dataloader",
...
# test reference in code execution
TEST_CASE_12 = [
    "optimizer",
...
# test replace reference with code execution result
TEST_CASE_13 = [
...
# test lambda function, should not execute the lambda function, just change the string with reference objects
TEST_CASE_15 = [
    "dataloader#<args>#collate_fn",
...
class TestConfigComponent(unittest.TestCase):
    ...
    def test_reference_ids(self, test_input, ref_ids):
        scanner = ModuleScanner(pkgs=[], modules=[])
        configer = ConfigComponent(id="test", config=test_input, module_scanner=scanner)
        ret = configer.get_referenced_ids()
