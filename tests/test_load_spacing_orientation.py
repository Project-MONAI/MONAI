...
class TestLoadSpacingOrientation(unittest.TestCase):
    ...
    def test_load_spacingd(
        ...
        res_dict = Spacingd(
            keys="image", pixdim=(1, 2, 3,), diagonal=True, mode="constant",
        )(data_dict)
        np.testing.assert_allclose(
            data_dict["image.affine"], res_dict["image.original_affine"],
        )
        anat = nibabel.Nifti1Image(data_dict["image"][0], data_dict["image.affine"],)
        ref = resample_to_output(anat, (1, 2, 3,),)
        np.testing.assert_allclose(
            res_dict["image.affine"], ref.affine,
        )
        np.testing.assert_allclose(
            res_dict["image"].shape[1:], ref.shape,
        )
        np.testing.assert_allclose(
            ref.get_fdata(), res_dict["image"][0],
        )

    @parameterized.expand(FILES)
    def test_load_spacingd_rotate(
        self, filename,
    ):
        data = {"image": filename}
        data_dict = LoadNiftid(keys="image")(data)
        ...
        if "anatomical" not in filename:
            ...
            # instead of ceil)
            np.testing.assert_allclose(
                ref.get_fdata()[..., :-1,], res_dict["image"][0],
            )
    ...
    def test_load_spacingd_non_diag(self,):
        ...
        res_dict = Spacingd(
            keys="image", pixdim=(1, 2, 3,), diagonal=False, mode="constant",
        )(data_dict)
        np.testing.assert_allclose(
            data_dict["image.affine"], res_dict["image.original_affine"],
        )
    ...
    def test_load_spacingd_rotate_non_diag(self,):
        ...
        data_dict = AddChanneld(keys="image")(data_dict)
        res_dict = Spacingd(
            keys="image", pixdim=(1, 2, 3,), diagonal=False, mode="nearest",
        )(data_dict)
        np.testing.assert_allclose(
            data_dict["image.affine"], res_dict["image.original_affine"],
        )
