...
class TestConvolution2D(TorchImageTestCase2D):
    ...
    def test_conv_only1(self,):
        conv = Convolution(
            2, self.input_channels, self.output_channels, conv_only=True,
        )
        out = conv(self.imt)
        expected_shape = (
            1,
    ...
    def test_transpose1(self,):
        conv = Convolution(
            2, self.input_channels, self.output_channels, is_transposed=True,
        )
        out = conv(self.imt)
        expected_shape = (
            1,
    ...
    def test_transpose2(self,):
        conv = Convolution(
            2, self.input_channels, self.output_channels, strides=2, is_transposed=True,
        )
        out = conv(self.imt)
        expected_shape = (
            1,
