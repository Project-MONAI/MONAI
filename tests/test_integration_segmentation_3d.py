...
def run_training_test(
    root_dir, device=torch.device("cuda:0"), cachedataset=False,
):
    ...
    if cachedataset:
        train_ds = monai.data.CacheDataset(
            data=train_files, transform=train_transforms, cache_rate=0.8,
        )
    else:
    ...
    for epoch in range(6):
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                writer.add_scalar(
                    "val_mean_dice", metric, epoch + 1,
                )
                # plot the last model output as GIF image in TensorBoard with the corresponding image and label
                plot_2d_or_3d_image(
                    val_images, epoch + 1, writer, index=0, tag="image",
                )
                plot_2d_or_3d_image(
                    val_labels, epoch + 1, writer, index=0, tag="label",
                )
                plot_2d_or_3d_image(
...
class IntegrationSegmentation3D(unittest.TestCase):
    def setUp(self,):
        ...
        for i in range(40):
            ...
            nib.save(
                n, os.path.join(self.data_dir, "img%i.nii.gz" % i,),
            )
            n = nib.Nifti1Image(seg, np.eye(4),)
            nib.save(
                n, os.path.join(self.data_dir, "seg%i.nii.gz" % i,),
            )
    ...
    def test_training(self,):
        ...
        for i in range(3):
            ...
            np.testing.assert_allclose(
                best_metric, 0.936915835738182, rtol=1e-4,
            )
            ...
            np.testing.assert_allclose(
                infer_metric, 0.9382847994565964, rtol=1e-4,
            )
            repeated[i].append(infer_metric)
            output_files = sorted(
                glob(os.path.join(self.data_dir, "output", "img*", "*.nii.gz",))
            )
            sums = [
                0.14089012145996094,
                0.15014171600341797,
