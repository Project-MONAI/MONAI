...
from monai.apps.detection.inferers import SlidingWindowMultiOutputInferer, sliding_window_inference_multioutput
...
class TestSlidingWindowMultiOutputInference(unittest.TestCase):
    ...
    def test_sliding_window_default(self, image_shape, roi_shape, sw_batch_size, overlap, mode, device):
        ...
        result = sliding_window_inference_multioutput(
            inputs.to(device), roi_shape, sw_batch_size, compute, overlap, mode=mode
        )
        np.testing.assert_string_equal(device.type, result.device.type)
        np.testing.assert_allclose(result.cpu().numpy(), expected_val)

        result = SlidingWindowMultiOutputInferer(roi_shape, sw_batch_size, overlap, mode)(inputs.to(device), compute)
        np.testing.assert_string_equal(device.type, result.device.type)
        np.testing.assert_allclose(result.cpu().numpy(), expected_val)
    ...
    def test_default_device(self):
        ...
        result = sliding_window_inference_multioutput(inputs, roi_shape, sw_batch_size, compute)
    ...
    def test_sw_device(self):
        ...
        result = sliding_window_inference_multioutput(inputs, roi_shape, sw_batch_size, compute, sw_device="cuda")
        np.testing.assert_string_equal(inputs.device.type, result.device.type)
        expected_val = np.ones((1, 3, 16, 15, 7), dtype=np.float32) + 1
        np.testing.assert_allclose(result.cpu().numpy(), expected_val)
    ...
    def test_sigma(self):
        ...
        result = SlidingWindowMultiOutputInferer(
            roi_shape, sw_batch_size, overlap=0.5, mode="gaussian", sigma_scale=1.0
    ...
    def test_cval(self):
        ...
        result = SlidingWindowMultiOutputInferer(roi_shape, sw_batch_size, overlap=0.5, mode="constant", cval=-1)(
            inputs, compute
        )
        np.testing.assert_allclose(result.cpu().numpy(), expected, rtol=1e-4)
