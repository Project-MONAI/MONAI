        ...
        for p in TEST_NDARRAYS:
            for q in TEST_NDARRAYS:
                ...
                result = crop(
                    {
                        ...
                        "seg": p(self.segn[0]),
                        "w": q(weight),
                        f"img_{DictPostFixes.META}": {"affine": None},
                    }
                )
                self.assertTrue(len(result) == n_samples)
                for c, e in zip(crop.centers, [[14, 32, 40], [41, 32, 40], [20, 32, 40]]):
                ...
                for i in range(n_samples):
                    ...
                    np.testing.assert_allclose(result[i][f"img_{DictPostFixes.META}"]["patch_index"], i)
                    np.testing.assert_allclose(result[i][f"seg_{DictPostFixes.META}"]["patch_index"], i)
