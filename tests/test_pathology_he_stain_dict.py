...
class TestExtractHEStainsD(unittest.TestCase):
    ...
    def test_transparent_image_cupy(self, image):
        ...
        if image is None:
            with self.assertRaises(TypeError):
                ExtractHEStainsD([key], cuda=True)({key: image})
        else:
...
class TestNormalizeHEStainsD(unittest.TestCase):
    ...
    def test_transparent_image_cupy(self, image):
        ...
        if image is None:
            with self.assertRaises(TypeError):
                NormalizeHEStainsD([key], cuda=True)({key: image})
        else:
            image = cp.array(image)
            with self.assertRaises(ValueError):
                NormalizeHEStainsD([key], cuda=True)({key: image})
    ...
    @skipUnless(has_cp, "CuPy is required.")
    def test_result_value_cupy(self, arguments, image, expected_data):
        """
        [For CuPy arrays]
        Test that an input image returns an expected normalized image.
        ...
        if image is None:
            ...
            image = cp.array(image)
            expected_data = cp.array(expected_data)
            result = NormalizeHEStainsD([key], cuda=True, **arguments)({key: image})
            cp.testing.assert_allclose(result[key], expected_data)
