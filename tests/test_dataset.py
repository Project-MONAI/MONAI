...
class TestDataset(unittest.TestCase):
    ...
    def test_shape(
        ...
        nib.save(
            test_image, os.path.join(tempdir, "test_image1.nii.gz",),
        )
        nib.save(
            test_image, os.path.join(tempdir, "test_label1.nii.gz",),
        )
        nib.save(
            test_image, os.path.join(tempdir, "test_extra1.nii.gz",),
        )
        nib.save(
            test_image, os.path.join(tempdir, "test_image2.nii.gz",),
        )
        nib.save(
            test_image, os.path.join(tempdir, "test_label2.nii.gz",),
        )
        nib.save(
            test_image, os.path.join(tempdir, "test_extra2.nii.gz",),
        )
        test_data = [
            {
        ...
        test_transform = Compose(
            [
                ...
                SimulateDelayd(
                    keys=["image", "label", "extra",], delay_time=[1e-7, 1e-6, 1e-5,],
                ),
            ]
        )
        ...
        self.assertTupleEqual(
            data1["label"].shape, expected_shape,
        )
        self.assertTupleEqual(
        ...
        dataset = Dataset(
            data=test_data, transform=LoadNiftid(keys=["image", "label", "extra",]),
        )
        data1_simple = dataset[0]
        data2_simple = dataset[1]

        self.assertTupleEqual(
            data1_simple["image"].shape, expected_shape,
        )
        self.assertTupleEqual(
            data1_simple["label"].shape, expected_shape,
        )
        self.assertTupleEqual(
            data1_simple["extra"].shape, expected_shape,
        )
        self.assertTupleEqual(
