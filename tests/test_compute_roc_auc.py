...
TEST_CASE_4 = [
    {"y_pred": torch.tensor([0.5, 0.5, 0.2, 8.3,]), "y": torch.tensor([0, 1, 0, 1,]),},
    0.875,
]
...
TEST_CASE_6 = [
    {
        "y_pred": torch.tensor(
            [[0.1, 0.9,], [0.3, 1.4,], [0.2, 0.1,], [0.1, 0.5,], [0.1, 0.5,],]
        ),
...
class TestComputeROCAUC(unittest.TestCase):
    ...
    def test_value(
        self, input_data, expected_value,
    ):
        result = compute_roc_auc(**input_data)
        np.testing.assert_allclose(
            expected_value, result, rtol=1e-5,
        )
