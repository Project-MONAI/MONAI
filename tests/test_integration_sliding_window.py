...
def run_test(
    batch_size, img_name, seg_name, output_dir, device=torch.device("cuda:0"),
):
    ds = NiftiDataset(
        [img_name],
        [seg_name],
    ...
    def _sliding_window_processor(
        ...
        with torch.no_grad():
            seg_probs = sliding_window_inference(
                img.to(device), roi_size, sw_batch_size, net,
            )
...
class TestIntegrationSlidingWindow(unittest.TestCase):
    def setUp(self,):
        ...
        (im, seg,) = create_test_image_3d(
            25, 28, 63, rad_max=10, noise_max=1, num_objs=4, num_seg_classes=1,
        )
    ...
    def test_training(self,):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            np.testing.assert_allclose(
                np.sum(output_image), 34070,
            )
            np.testing.assert_allclose(
                output_image.shape, (28, 25, 63, 1,),
            )
