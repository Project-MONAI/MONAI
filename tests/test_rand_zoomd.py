...
class TestRandZoomd(NumpyImageTestCase2D):
    ...
    def test_correct_results(
        self, min_zoom, max_zoom, order, mode, cval, prefilter, use_gpu, keep_size,
    ):
        key = "img"
    ...
    def test_gpu_zoom(
        self, min_zoom, max_zoom, order, mode, cval, prefilter,
    ):
        key = "img"
    ...
    def test_keep_size(self,):
        key = "img"
        random_zoom = RandZoomd(
            key, prob=1.0, min_zoom=0.6, max_zoom=0.7, keep_size=True,
        )
        zoomed = random_zoom({key: self.imt[0]})
    ...
    def test_invalid_inputs(
        self, _, min_zoom, max_zoom, order, raises,
    ):
        key = "img"
