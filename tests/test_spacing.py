...
TEST_CASES = [
    ...
    [
        ...
        {
            "affine": np.array(
                [[2, 1, 0, 4,], [-1, -3, 0, 5,], [0, 0, 2.0, 5,], [0, 0, 0, 1,],],
            ),
        },
        np.zeros((1, 3, 1, 2,)),
    ],
    ...
    [
        ...
        {},
        np.array(
            [[[0, 1, 2, 3,], [0, 0, 0, 0,],], [[12, 13, 14, 15,], [0, 0, 0, 0,],],]
        ),
    ],
    [
        {"pixdim": (1.0, 1.0,)},
    ...
    [
        ...
        {
            "affine": np.array(
                [[-4, 0, 0, 4,], [0, 5, 0, -5,], [0, 0, 6, -6,], [0, 0, 0, 1,],]
            )
        },
        np.arange(24).reshape((1, 2, 3, 4,)),  # data
    ],
    [
        ...
        {
            "affine": np.array(
                [[-4, 0, 0, 4,], [0, 5, 0, 0,], [0, 0, 6, 0,], [0, 0, 0, 1,],]
            )
        },
        np.array(
            [
    ...
    [
        ...
        {
            "affine": np.array(
                [[-4, 0, 0, 0,], [0, 5, 0, 0,], [0, 0, 6, 0,], [0, 0, 0, 1,],]
            ),
            "interp_order": 0,
        },
        np.array(
...
class TestSpacingCase(unittest.TestCase):
    ...
    def test_spacing(
        self, init_param, img, data_param, expected_output,
    ):
        res = Spacing(**init_param)(img, **data_param,)
        np.testing.assert_allclose(
            res[0], expected_output, atol=1e-6,
        )
        if "original_affine" in data_param:
            np.testing.assert_allclose(
                res[1], data_param["original_affine"],
            )
        np.testing.assert_allclose(
            init_param["pixdim"],
