...
class TestSpacingDCase(unittest.TestCase):
    def test_spacingd_3d(self,):
        data = {
            "image": np.ones((2, 10, 15, 20,)),
            "image.affine": np.eye(4),
        }
        spacing = Spacingd(keys="image", pixdim=(1, 2, 1.4,),)
        res = spacing(data)
        self.assertEqual(
            ("image", "image.affine",), tuple(sorted(res)),
        )
        np.testing.assert_allclose(
            res["image"].shape, (2, 10, 8, 15,),
        )
        np.testing.assert_allclose(
            res["image.affine"], np.diag([1, 2, 1.4, 1.0,]),
        )

    def test_spacingd_2d(self,):
        data = {
            "image": np.ones((2, 10, 20,)),
            "image.affine": np.eye(3),
        }
        spacing = Spacingd(keys="image", pixdim=(1, 2, 1.4,),)
        res = spacing(data)
        self.assertEqual(
        ...
        np.testing.assert_allclose(
            res["image"].shape, (2, 10, 10,),
        )
        np.testing.assert_allclose(
            res["image.affine"], np.diag((1, 2, 1,)),
        )
    ...
    def test_spacingd_1d(self,):
        ...
        res = spacing(data)
        self.assertEqual(
            ("image", "image.affine", "image.original_affine",), tuple(sorted(res)),
        )
        np.testing.assert_allclose(
            res["image"].shape, (2, 136,),
        )
        np.testing.assert_allclose(
            res["image.affine"], np.diag((0.2, 2, 1, 1,)),
        )
        np.testing.assert_allclose(
            res["image.original_affine"], np.diag((3, 2, 1, 1,)),
        )
    ...
    def test_interp_all(self,):
        ...
        res = spacing(data)
        self.assertEqual(
            ("image", "image.affine", "seg", "seg.affine",), tuple(sorted(res)),
        )
        np.testing.assert_allclose(
            res["image"].shape, (2, 46,),
        )
        np.testing.assert_allclose(
            res["image.affine"], np.diag((0.2, 1, 1, 1,)),
        )
