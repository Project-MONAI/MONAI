...
class TestPngWrite(unittest.TestCase):
    def test_write_gray(self):
        with tempfile.TemporaryDirectory() as out_dir:
            ...
            writer_obj = PILWriter(output_dtype=np.uint8).set_data_array(img, channel_dim=None)
    ...
    def test_write_gray_1height(self):
        with tempfile.TemporaryDirectory() as out_dir:
            ...
            writer_obj = PILWriter(output_dtype=np.uint16, scale=65535).set_data_array(img, channel_dim=None)
            writer_obj.write(image_name, format="PNG")
            out = np.asarray(Image.open(image_name))
            out = np.moveaxis(out, 0, 1)
            np.testing.assert_allclose(out, img_save_val)
    ...
    def test_write_gray_1channel(self):
        with tempfile.TemporaryDirectory() as out_dir:
            ...
            writer_obj = PILWriter(output_dtype=np.uint8, scale=255).set_data_array(img, channel_dim=None)
            writer_obj.write(image_name, format="PNG")
            out = np.asarray(Image.open(image_name))
            out = np.moveaxis(out, 0, 1)
            np.testing.assert_allclose(out, img_save_val)
    ...
    def test_write_rgb(self):
        ...
        with tempfile.TemporaryDirectory() as out_dir:
            ...
            img_save_val = (255 * img).astype(np.uint8)
            writer_obj = PILWriter(output_dtype=np.uint8).set_data_array(img, channel_dim=-1)
    ...
    def test_write_output_shape(self):
        with tempfile.TemporaryDirectory() as out_dir:
            ...
            writer_obj = (
                PILWriter(output_dtype=np.uint8)
                .set_data_array(img, channel_dim=-1)
                .set_metadata({"spatial_shape": (4, 4)}, scale=255)
            )
            writer_obj.write(image_name, format="PNG")
            out = np.asarray(Image.open(image_name))
