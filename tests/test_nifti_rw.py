...
class TestNiftiLoadRead(unittest.TestCase):
    ...
    def test_orientation(self, array, affine, reader_param, expected):
        ...
        if header is not None:
            writer_obj = NibabelWriter().set_data_array(data_array, channel_dim=None).set_metadata(header)
            writer_obj.write(test_image, verbose=True)
        elif affine is not None:
            writer_obj = NibabelWriter().set_data_array(data_array, channel_dim=None).set_metadata({"affine": affine})
            writer_obj.write(test_image, verbose=True)
    ...
    def test_consistency(self):
        ...
        writer_obj = (
            NibabelWriter()
            .set_data_array(data[0], channel_dim=None)
            .set_metadata(
                meta_dict={"affine": new_affine, "original_affine": original_affine},
                mode="nearest",
                padding_mode="border",
            )
        )
        ...
        writer_obj = (
            NibabelWriter()
            .set_data_array(data[0], channel_dim=None)
            .set_metadata(meta_dict={"affine": new_affine, "original_affine": original_affine}, resample=False)
        )
        writer_obj.write(test_image, verbose=True)
        saved = nib.load(test_image)
    ...
    def test_write_2d(self):
        with tempfile.TemporaryDirectory() as out_dir:
            ...
            for p in TEST_NDARRAYS:
                ...
                writer_obj = (
                    NibabelWriter()
                    .set_data_array(img, channel_dim=None)
                    .set_metadata({"affine": np.diag([1]), "original_affine": np.diag([1.4])})
                )
                ...
                writer_obj = (
                    NibabelWriter()
                    .set_data_array(img, channel_dim=None)
                    .set_metadata({"affine": np.diag([1, 1, 1, 3, 3]), "original_affine": np.diag([1.4, 2.0, 1, 3, 5])})
                )
                writer_obj.write(image_name, verbose=True)
                out = nib.load(image_name)
        ...
        with tempfile.TemporaryDirectory() as out_dir:
            ...
            for p in TEST_NDARRAYS:
                ...
                writer_obj = (
                    NibabelWriter()
                    .set_data_array(img, channel_dim=None)
                    .set_metadata({"affine": np.diag([1, 1, 1, 3, 3]), "original_affine": np.diag([1.4, 2.0, 2, 3, 5])})
                )
                writer_obj.write(image_name, verbose=True)
                out = nib.load(image_name)
                np.testing.assert_allclose(out.get_fdata(), [[[0, 2, 4]]])
    ...
    def test_write_4d(self):
        with tempfile.TemporaryDirectory() as out_dir:
            ...
            for p in TEST_NDARRAYS:
                ...
                writer_obj = (
                    NibabelWriter()
                    .set_data_array(img, channel_dim=-1)
                    .set_metadata({"affine": np.diag([1.4, 1]), "original_affine": np.diag([1, 1.4, 1])})
                )
                writer_obj.write(image_name, verbose=True)
                out = nib.load(image_name)
                np.testing.assert_allclose(out.get_fdata(), [[[[0, 1], [2, 3], [4, 5]]]])
                ...
                writer_obj = (
                    NibabelWriter()
                    .set_data_array(img, channel_dim=-1, squeeze_end_dims=False)
                    .set_metadata({"affine": np.diag([1, 1, 1, 3, 3]), "original_affine": np.diag([1.4, 2.0, 2, 3, 5])})
                )
                writer_obj.write(image_name, verbose=True)
                out = nib.load(image_name)
    ...
    def test_write_5d(self):
        with tempfile.TemporaryDirectory() as out_dir:
            ...
            for p in TEST_NDARRAYS:
                ...
                writer_obj = (
                    NibabelWriter()
                    .set_data_array(img, channel_dim=-1, squeeze_end_dims=False, spatial_ndim=None)
                    .set_metadata({"affine": np.diag([1]), "original_affine": np.diag([1.4])})
                )
                writer_obj.write(image_name, verbose=True)
                out = nib.load(image_name)
