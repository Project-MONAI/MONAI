...
class TestNiftiLoadRead(unittest.TestCase):
    ...
    def test_orientation(
        ...
        if header is not None:
            ...
            write_nifti(
                data_array, test_image, affine,
            )
        else:
            write_nifti(
                data_array, test_image,
            )
    ...
    def test_consistency(self,):
        ...
        (data, original_affine, new_affine,) = Spacing([0.8, 0.8, 0.8,])(
            data[None], header["affine"], interp_order=0,
        )
        ...
        saved_data = saved.get_fdata()
        np.testing.assert_allclose(
            saved_data, np.arange(64).reshape(1, 8, 8,), atol=1e-7,
        )
        if os.path.exists(test_image):
            os.remove(test_image)
        write_nifti(
    ...
    def test_write_1d(self,):
        with tempfile.TemporaryDirectory() as out_dir:
            ...
            np.testing.assert_allclose(
                out.get_fdata(), [0, 1, 3, 0,],
            )
            np.testing.assert_allclose(
                out.affine, np.diag([1.4, 1, 1, 1,]),
            )
        with tempfile.TemporaryDirectory() as out_dir:
            image_name = os.path.join(out_dir, "test.nii.gz",)
            img = np.arange(5).reshape(-1)
            write_nifti(
                img, image_name, affine=[[1]], target_affine=[[1.4]],
            )
            out = nib.load(image_name)
            np.testing.assert_allclose(
                out.get_fdata(), [0, 1, 3, 0,],
        ...
        with tempfile.TemporaryDirectory() as out_dir:
            ...
            out = nib.load(image_name)
            np.testing.assert_allclose(
                out.get_fdata(), np.arange(5).reshape(-1),
            )
            np.testing.assert_allclose(
                out.affine, np.diag([1.5, 1, 1, 1,]),
            )
    ...
    def test_write_2d(self,):
        with tempfile.TemporaryDirectory() as out_dir:
            ...
            write_nifti(
                img, image_name, affine=np.diag([1]), target_affine=np.diag([1.4]),
            )
            out = nib.load(image_name)
            np.testing.assert_allclose(
                out.get_fdata(), [[0, 1, 2,], [0, 0, 0,],],
            )
            np.testing.assert_allclose(
                out.affine, np.diag([1.4, 1, 1, 1,]),
            )
