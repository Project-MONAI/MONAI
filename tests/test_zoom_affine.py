...
class TestZoomAffine(unittest.TestCase):
    ...
    def test_correct(
        self, affine, scale, expected,
    ):
        output = zoom_affine(affine, scale, diagonal=False,)
        ornt_affine = nib.orientations.ornt2axcodes(
            nib.orientations.io_orientation(output)
        )
        ornt_output = nib.orientations.ornt2axcodes(
            nib.orientations.io_orientation(affine)
        )
        np.testing.assert_array_equal(
            ornt_affine, ornt_output,
        )
