...
from monai.transforms.intensity.array import GetForegroundPatches
...
class TestGetForegroundPatches(unittest.TestCase):
    @parameterized.expand(TEST_CASES)
    def test_grid_patch(self, in_type, input_parameters, image, expected_image, expected_mask):
        input_image = in_type(image)
        converter = GetForegroundPatches(**input_parameters)
        images, masks = converter(input_image)

        self.assertEqual(len(images), len(expected_image))
        self.assertEqual(len(masks), len(expected_mask))
        for output_patch, expected_patch in zip(images, expected_image):
            assert_allclose(output_patch, expected_patch, type_test=False)

        for output_patch, expected_patch in zip(masks, expected_mask):
            assert_allclose(output_patch, expected_patch, type_test=False)
