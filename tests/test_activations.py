...
for p in TEST_NDARRAYS:
    TESTS1.append(
        [
            ...
            p(torch.tensor([[[0.0, 1.0], [2.0, 3.0]]])),
            p(torch.tensor([[[0.5000, 0.7311], [0.8808, 0.9526]]])),
            (1, 2, 2),
        ]
    )
    TESTS1.append(
        [
            ...
            p(torch.tensor([[[0.0, 1.0]], [[2.0, 3.0]]])),
            p(torch.tensor([[[0.1192, 0.1192]], [[0.8808, 0.8808]]])),
            (2, 1, 2),
        ]
    )
    TESTS1.append(
        [
            ...
            p(torch.tensor([[[0.0, 1.0], [2.0, 3.0]]])),
            p(torch.tensor([[[0.0000, 0.7616], [0.9640, 0.9951]]])),
            (1, 2, 2),
        ]
    )

for p in TEST_NDARRAYS:
    # these tests require torch.Tensor
    if p == np.array:
        continue
    TESTS2.append(
        [
            "swish",
            p(torch.tensor([[[-10, -8, -6, -4, -2], [0, 2, 4, 6, 8]]], dtype=torch.float32)),
            p(
                torch.tensor(
                    [
                        [
