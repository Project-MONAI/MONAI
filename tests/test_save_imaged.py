...
TEST_CASE_1 = [
    {"img": torch.randint(0, 255, (1, 2, 3, 4)), f"img_{DictPostFixes.META}": {"filename_or_obj": "testfile0.nii.gz"}},
    ".nii.gz",
    False,
]
...
TEST_CASE_2 = [
    {
        ...
        f"img_{DictPostFixes.META}": {"filename_or_obj": "testfile0.nii.gz"},
        "patch_index": 6,
    },
    ".nii.gz",
    False,
...
class TestSaveImaged(unittest.TestCase):
    ...
    def test_saved_content(self, test_data, output_ext, resample):
        with tempfile.TemporaryDirectory() as tempdir:
            trans = SaveImaged(
                keys=["img", "pred"],
                meta_keys=f"img_{DictPostFixes.META}",
                output_dir=tempdir,
            ...
            patch_index = test_data[f"img_{DictPostFixes.META}"].get("patch_index", None)
