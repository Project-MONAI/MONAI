...
class TestRand3DElastic(unittest.TestCase):
    ...
    def test_rand_3d_elastic(
        ...
        result = g(**input_data)
        self.assertEqual(
            torch.is_tensor(result), torch.is_tensor(expected_val),
        )
        if torch.is_tensor(result):
            np.testing.assert_allclose(
                result.cpu().numpy(), expected_val.cpu().numpy(), rtol=1e-4, atol=1e-4,
            )
        else:
            np.testing.assert_allclose(
                result, expected_val, rtol=1e-4, atol=1e-4,
            )
