...
class LocalNormalizedCrossCorrelation(_Loss):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            dim: number of spatial dimensions, {``1``, ``2``, ``3``}. Defaults to 3.
        ...
        if self.dim not in [1, 2, 3]:
            raise ValueError(f'Unsupported dim: {self.dim}-d, only 1-d, 2-d, and 3-d inputs are supported')
    ...
    def make_rectangular_kernel(self):
        shape = [1, self.in_channels] + [self.kernel_size] * self.dim
        return torch.ones(shape, dtype=torch.float), self.kernel_size ** self.dim, int((self.kernel_size - 1) / 2)
    ...
    def make_triangular_kernel(self):
        ...
        f2 = torch.ones(
            [1, self.in_channels] + [fsize] * self.dim,
            dtype=torch.float
        ) / fsize  # (1, in_channels, D, H, W)
        # (1, 1, D, H, W) -> (1, in_channels, D, H, W)
        fn = conv_dict[self.dim]
    ...
    def make_gaussian_kernel(self):
        ...
        grid_dim = torch.arange(0, self.kernel_size)
        grid_dim_ch = torch.arange(0, self.in_channel)

        if self.dim == 1:
            grid = torch.meshgrid(grid_dim_ch, grid_dim)
        elif self.dim == 2:
            grid = torch.meshgrid(grid_dim_ch, grid_dim, grid_dim)
        elif self.dim == 3:
            grid = torch.meshgrid(grid_dim_ch, grid_dim, grid_dim, grid_dim)
        else:
            raise ValueError

        grid = torch.stack(grid, dim=-1).to(dtype=torch.float)
        kernel = torch.exp(
            -torch.sum(torch.square(grid - mean), dim=-1) / (2 * sigma ** 2)
        ).unsqueeze(0)  # (1, in_channel, kernel_size, kernel_size, kernel_size)
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        p_sum = fn(input, weight=self.kernel, padding=self.paddin)
        t2_sum = fn(t2, weight=self.kernel, padding=self.paddin)
        p2_sum = fn(p2, weight=self.kernel, padding=self.paddin)
        tp_sum = fn(tp, weight=self.kernel, padding=self.paddin)
        ...
        if self.reduction == LossReduction.SUM.value:
            return torch.sum(ncc)  # sum over the batch and channel dims
        if self.reduction == LossReduction.NONE.value:
            return ncc
        if self.reduction == LossReduction.MEAN.value:
            return torch.mean(ncc)  # average over the batch and channel dims
