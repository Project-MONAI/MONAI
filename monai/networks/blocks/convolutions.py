...
class Convolution(nn.Sequential):
    ...
    def __init__(
        ...
        if not conv_only:
            self.add_module(
                "norm", norm_type(out_channels, **norm_args,),
            )
            if dropout:
                self.add_module(
                    "dropout", drop_type(**drop_args),
                )

            self.add_module(
                "act", act_type(**act_args),
            )
