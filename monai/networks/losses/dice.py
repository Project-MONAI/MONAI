...
from monai.networks.utils import oneHot
...
class DiceLoss(_Loss):
    ...
    def __init__(self, includeBackground=True):
        """
        If `includeBackground` is False channel index 0 (background category) is excluded from the calculation.
        """
        super().__init__()
    ...
    def forward(self, pred, ground, smooth=1e-5):
        ...
        if pred.shape[1] == 1:  # binary dice loss, use sigmoid activation
            ...
            tsum = oneHot(ground, pred.shape[1])  # BCHW(D) -> BCHW(D)N
