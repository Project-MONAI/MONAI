...
def densenet121(**kwargs,):
    model = DenseNet(
        init_features=64, growth_rate=32, block_config=(6, 12, 24, 16,), **kwargs,
    )
    return model
...
def densenet169(**kwargs,):
    model = DenseNet(
        init_features=64, growth_rate=32, block_config=(6, 12, 32, 32,), **kwargs,
    )
    return model
...
def densenet201(**kwargs,):
    model = DenseNet(
        init_features=64, growth_rate=32, block_config=(6, 12, 48, 32,), **kwargs,
    )
    return model
...
def densenet264(**kwargs,):
    model = DenseNet(
        init_features=64, growth_rate=32, block_config=(6, 12, 64, 48,), **kwargs,
    )
    return model
...
class _DenseLayer(nn.Sequential):
    def __init__(
        self, spatial_dims, in_channels, growth_rate, bn_size, dropout_prob,
    ):
        ...
        dropout_type = Dropout[
            Dropout.DROPOUT, spatial_dims,
        ]

        self.add_module(
            "norm1", norm_type(in_channels),
        )
        self.add_module(
        ...
        if dropout_prob > 0:
            self.add_module(
                "dropout", dropout_type(dropout_prob),
            )
...
class _DenseBlock(nn.Sequential):
    def __init__(
        self, spatial_dims, layers, in_channels, bn_size, growth_rate, dropout_prob,
    ):
        super(_DenseBlock, self,).__init__()
        for i in range(layers):
            layer = _DenseLayer(
                spatial_dims, in_channels, growth_rate, bn_size, dropout_prob,
            )
...
class _Transition(nn.Sequential):
    def __init__(
        ...
        self.add_module(
            "pool", pool_type(kernel_size=2, stride=2,),
        )
    ...
    def __init__(
        ...
        for (i, num_layers,) in enumerate(block_config):
            ...
            if i == len(block_config) - 1:
                self.features.add_module(
                    "norm5", norm_type(in_channels),
                )
            else:
                _out_channels = in_channels // 2
                trans = _Transition(
                    spatial_dims, in_channels=in_channels, out_channels=_out_channels,
                )
                self.features.add_module(
