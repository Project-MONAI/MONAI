...
class SpatialPad(Transform):
    ...
    def __init__(
        self, spatial_size, method="symmetric", mode="constant",
    ):
        assert isinstance(
            spatial_size, (list, tuple,),
        ), "spatial_out_size must be list or tuple."
    ...
    def _determine_data_pad_width(
        ...
        if self.method == "symmetric":
            ...
            return [
                (0, max(self.spatial_size[i] - data_shape[i], 0,),)
                for i in range(len(self.spatial_size))
            ]
...
class SpatialCrop(Transform):
    ...
    def __init__(
        self, roi_center=None, roi_size=None, roi_start=None, roi_end=None,
    ):
        """
        Args:
        ...
        if roi_center is not None and roi_size is not None:
            ...
            assert (
                roi_start is not None and roi_end is not None
            ), "roi_start and roi_end must be provided."
            self.roi_start = np.asarray(roi_start, dtype=np.uint16,)
            self.roi_end = np.asarray(roi_end, dtype=np.uint16,)

        assert np.all(
            self.roi_start >= 0
        ), "all elements of roi_start must be greater than or equal to 0."
    ...
    def __call__(
        ...
        assert np.all(
            max_end[:sd] >= self.roi_start[:sd]
        ), "roi start out of image space."
        assert np.all(max_end[:sd] >= self.roi_end[:sd]), "roi end out of image space."

        slices = [slice(None)] + [
            slice(s, e,) for s, e in zip(self.roi_start[:sd], self.roi_end[:sd],)
        ]
...
class RandSpatialCrop(
    ...
    def __init__(
        self, roi_size, random_center=True, random_size=True,
    ):
        self.roi_size = roi_size
...
class CropForeground(Transform):
    ...
    def __init__(
        self, select_fn=lambda x: x > 0, channel_indexes=None, margin=0,
    ):
        """
        Args:
    ...
    def __call__(
        ...
        (box_start, box_end,) = generate_spatial_bounding_box(
            img, self.select_fn, self.channel_indexes, self.margin,
        )
