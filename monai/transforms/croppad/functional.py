...
def pad_func(img_t, to_pad_, mode, kwargs, transform_info):
    ...
    if do_pad:
        img_t = monai.transforms.Pad.pad_nd(img_t, to_pad_, mode, **kwargs)
    img_t = convert_to_tensor(img_t, track_meta=get_track_meta())
    return img_t.copy_meta_from(meta_info) if isinstance(img_t, MetaTensor) else img_t
