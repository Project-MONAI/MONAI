...
class Spacing(Transform):
    ...
    def __init__(
        self, pixdim, diagonal=False, mode="constant", cval=0, dtype=None,
    ):
        """
        Args:
    ...
    def __call__(
        self, data_array, affine=None, interp_order=3,
    ):
        """
        Args:
        ...
        if self.as_closest_canonical:
            ...
            if len(dst) < sr:
                raise ValueError(
                    ...
                    ' given the data array is in spatial {0}D, got "{1}"'.format(
                        sr, self.axcodes,
                    )
                )
...
class RandRotate90(
    ...
    def __init__(
        self, prob=0.1, max_k=3, spatial_axes=(0, 1,),
    ):
        """
        Args:
...
class RandFlip(
    ...
    def __init__(
        self, prob=0.1, spatial_axis=None,
    ):
        self.prob = prob
    ...
    def __init__(
        ...
        if hasattr(min_zoom, "__iter__",) and hasattr(max_zoom, "__iter__",):
            assert len(min_zoom) == len(
                max_zoom
            ), "min_zoom and max_zoom must have same length."
    ...
    def __call__(
        ...
        if grid is None:
            if spatial_size is not None:
                ...
                raise ValueError(
                    "Either specify a grid or a spatial size to create a grid from."
                )
        ...
        grid = (
            torch.as_tensor(np.ascontiguousarray(grid))
            if not torch.is_tensor(grid)
            else grid.detach().clone()
        )
        if self.device:
            grid = grid.to(self.device)
        grid = (affine.float() @ grid.reshape((grid.shape[0], -1,)).float()).reshape(
            [-1] + list(grid.shape[1:])
        )
...
class RandAffineGrid(
    ...
    def randomize(self,):
        if self.rotate_range:
            self.rotate_params = [
                self.R.uniform(-f, f,) for f in self.rotate_range if f is not None
            ]
        if self.shear_range:
            self.shear_params = [
                self.R.uniform(-f, f,) for f in self.shear_range if f is not None
            ]
        if self.translate_range:
            self.translate_params = [
                self.R.uniform(-f, f,) for f in self.translate_range if f is not None
            ]
        if self.scale_range:
            self.scale_params = [
                self.R.uniform(-f, f,) + 1.0 for f in self.scale_range if f is not None
            ]
...
class RandDeformGrid(
    ...
    def __init__(
        self, spacing, magnitude_range, as_tensor_output=True, device=None,
    ):
        """
        Args:
...
class Resample(Transform):
    def __init__(
        self, padding_mode="zeros", as_tensor_output=False, device=None,
    ):
        """
...
class Affine(Transform):
    ...
    def __init__(
        ...
        self.resampler = Resample(
            padding_mode=padding_mode, as_tensor_output=as_tensor_output, device=device,
        )
        self.spatial_size = spatial_size
        self.mode = mode

    def __call__(
        self, img, spatial_size=None, mode=None,
    ):
        """
        Args:
            img (ndarray or tensor): shape must be (num_channels, H, W[, D]),
...
class RandAffine(
    ...
    def set_random_state(
        ...
        self.rand_affine_grid.set_random_state(
            seed, state,
        )
        super().set_random_state(
            seed, state,
        )
        return self
...
class Rand2DElastic(
    ...
    def set_random_state(
        ...
        self.deform_grid.set_random_state(
            seed, state,
        )
        self.rand_affine_grid.set_random_state(
            seed, state,
    ...
    def __call__(
        ...
        if self.do_transform:
            ...
            grid = torch.nn.functional.interpolate(
                grid[None], spatial_size, mode="bicubic", align_corners=False,
            )[0]
        else:
            grid = create_grid(spatial_size)
