...
from monai.utils.misc import ensure_tuple, validate_kwargs
...
class Spacing(Transform):
    ...
    def __call__(self, data: Iterable, *args, **kwargs):
        """
        Args:
            data (numpy.ndarray): in shape (num_channels, H[, W, ...]).
            affine (matrix): (N+1)x(N+1) original affine matrix for spatially ND `data`. Default is identity.
            interp_order (`nearest|bilinear`): The interpolation mode. Default is ``self.interp_order``.
                See also: https://pytorch.org/docs/stable/nn.functional.html#grid-sample.
            mode (`zeros|border|reflection`):
                The mode parameter determines how the input array is extended beyond its boundaries.
                Default is ``self.mode``.
            dtype (Optional[np.dtype]): output array data type. Default is ``self.dtype``.
        Returns:
            data (resampled into `self.pixdim`), original pixdim, current pixdim.
        """
        reference_args: OrderedDict = OrderedDict({"affine": None, "interp_order": None, "mode": None, "dtype": None})
        produced_args: Dict[str, Any] = validate_kwargs(args, kwargs, reference_args)

        affine = produced_args["affine"]
        interp_order: Optional[str] = produced_args["interp_order"]
...
class Resize(Transform):
    ...
    def __call__(self, data: Iterable, *args, **kwargs):
        """
        Args:
            data (numpy.ndarray): channel first array, must have shape: (num_channels, H[, W, ..., ])
            order (Optional[int]): Order of spline interpolation. Default is ``self.order``.
            mode (Optional[str]): Points outside boundaries are filled according to given mode.
                Options are 'constant', 'edge', 'symmetric', 'reflect', 'wrap'. Default is ``self.mode``.
            cval (Optional[float]): Used with mode 'constant', the value outside image boundaries. Default is ``self.cval``.
            clip (Optional[bool]): Whether to clip range of output values after interpolation. Default is ``self.clip``.
            preserve_range (Optional[bool]): Whether to keep original range of values. Default is ``self.preserve_range``.
                If False, input is converted according to conventions of img_as_float. See
                https://scikit-image.org/docs/dev/user_guide/data_types.html.
            anti_aliasing (Optional[bool]): Whether to apply a gaussian filter to image before down-scaling.
                Default is ``self.anti_aliasing``.
        """
        reference_args: OrderedDict = OrderedDict(
            {"order": None, "mode": None, "cval": None, "clip": None, "preserve_range": None, "anti_aliasing": None}
        )
        produced_args: Dict[str, Any] = validate_kwargs(args, kwargs, reference_args)

        order: Optional[int] = produced_args["order"]
        mode: Optional[str] = produced_args["mode"]
    ...
    def __call__(self, data: Iterable, *args, **kwargs):
        ...
        Args:
            ...
            mode (Optional[str]): Points outside boundaries are filled according to given mode.
                Options are 'constant', 'edge', 'symmetric', 'reflect', 'wrap'. Default is ``self.mode``.
            cval (Optional[float]): Used with mode 'constant', the value outside image boundaries. Default is ``self.cval``.
            prefilter (Optional[bool]): Apply spline_filter before interpolation. Default is ``self.prefilter``.
        """
        reference_args: OrderedDict = OrderedDict({"order": None, "mode": None, "cval": None, "prefilter": None})
        produced_args: Dict[str, Any] = validate_kwargs(args, kwargs, reference_args)
...
class Resample(Transform):
    ...
    def __call__(self, data: Iterable, *args, **kwargs):
        """
        Args:
            data (Union[numpy.ndarray, torch.Tensor]): shape must be (num_channels, H, W[, D]).
            grid (Union[numpy.ndarray, torch.Tensor]): shape must be (3, H, W) for 2D or (4, H, W, D) for 3D.
            padding_mode ('zeros'|'border'|'reflection'): mode of handling out of range indices. Default is ``self.padding_mode``.
            mode ('nearest'|'bilinear'): interpolation order. Default is ``self.mode``.
        """
        reference_args: OrderedDict = OrderedDict({"grid": "__from_input__", "padding_mode": None, "mode": None})
        produced_args: Dict[str, Any] = validate_kwargs(args, kwargs, reference_args)

        grid: Union[np.ndarray, torch.Tensor] = produced_args["grid"]
        padding_mode: Optional[str] = produced_args["padding_mode"]
        mode: Optional[str] = produced_args["mode"]

        assert isinstance(data, (np.ndarray, torch.Tensor)), "data must be a numpy.ndarray or torch.Tensor."

        if not torch.is_tensor(data):
            data = torch.as_tensor(np.ascontiguousarray(data))
        grid = torch.tensor(grid) if not torch.is_tensor(grid) else grid.detach().clone()
        if self.device:
            data = data.to(self.device)  # pytype: disable=attribute-error
            grid = grid.to(self.device)

        for i, dim in enumerate(data.shape[1:]):  # pytype: disable=attribute-error
            grid[i] = 2.0 * grid[i] / (dim - 1.0)
        grid = grid[:-1] / grid[-1:]
        index_ordering: List[int] = [x for x in range(data.ndim - 2, -1, -1)]  # pytype: disable=attribute-error
...
class Affine(Transform):
    ...
    def __call__(self, data: Iterable, *args, **kwargs):
        ...
        return self.resampler(
            data=data, grid=grid, padding_mode=padding_mode or self.padding_mode, mode=mode or self.mode
        )
...
class Rand2DElastic(Randomizable, Transform):
    ...
    def __call__(self, data: Iterable, *args, **kwargs):
        """
        Args:
            data (Union[numpy.ndarray, torch.Tensor]): shape must be (num_channels, H, W),
            spatial_size (2 ints): specifying output image spatial size [h, w].
            padding_mode ('zeros'|'border'|'reflection'): mode of handling out of range indices.
                Defaults to ``self.padding_mode``.
