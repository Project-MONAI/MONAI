...
class SpatialResample(InvertibleTransform, LazyTransform):
    ...
    def __init__(
        ...
        Args:
            ...
            lazy_evaluation: a flag to indicate whether this transform should execute lazily or not.
                Defaults to False
        """
        LazyTransform.__init__(self, lazy_evaluation=lazy_evaluation)
        self.mode = mode
    ...
    def __call__(
        ...
        Args:
            ...
            lazy_evaluation: a flag to indicate whether this transform should execute lazily or not
                during this call. Setting this to False or True overrides the ``lazy_evaluation`` flag set
        ...
        lazy_ = self.lazy_evaluation if lazy_evaluation is None else lazy_evaluation
    ...
    def __call__(  # type: ignore
        ...
        if not lazy_:
            ...
            if isinstance(img, MetaTensor) and isinstance(img_dst, MetaTensor):
                ...
                for k in ("affine", "spatial_shape"):  # keys that don't copy from img_dst in lazy_evaluation evaluation
...
class Spacing(InvertibleTransform, LazyTransform):
    ...
    def __init__(
        ...
        self.sp_resample = SpatialResample(
            mode=mode, padding_mode=padding_mode, align_corners=align_corners, dtype=dtype, lazy_evaluation=lazy_evaluation
        )
...
class Orientation(InvertibleTransform, LazyTransform):
    ...
    def __call__(self, data_array: torch.Tensor, lazy_evaluation: bool | None = None) -> torch.Tensor:
        """
        ...
        lazy_ = self.lazy_evaluation if lazy_evaluation is None else lazy_evaluation
        return orientation(data_array, affine_np, spatial_ornt, lazy_evaluation=lazy_, transform_info=self.get_transform_info())
...
class Flip(LazyTransform, InvertibleTransform):
    ...
    def __call__(
        ...
        img_t = flip(img, spatial_axis_, lazy_evaluation=lazy_)
...
class Resize(LazyTransform, InvertibleTransform):
    ...
    def __call__(
        ...
        img_t = resize(img, self.spatial_size, self.size_mode, mode_, align_corners_, anti_aliasing_,
                       anti_aliasing_sigma_, self.dtype, lazy_evaluation=lazy_)
...
class Rotate(InvertibleTransform, LazyTransform):
    ...
    def __call__(
        ...
        img_t = rotate(img, self.angle, keep_size, mode_, padding_mode_, align_corners_, dtype_, lazy_evaluation=lazy_)
...
class Zoom(LazyTransform, InvertibleTransform):
    ...
    def __call__(
        ...
        img_t = zoom(img, self.zoom, mode_, padding_mode_, align_corners_, self.keep_size, img.dtype, lazy_evaluation=lazy_)
...
class Rotate90(InvertibleTransform, LazyTransform):
    ...
    def __call__(
        ...
        """
        lazy_ = self.lazy_evaluation if lazy_evaluation is None else lazy_evaluation
        img_t = rotate90(img, self.k, self.spatial_axes, lazy_evaluation=lazy_)
...
class RandRotate90(RandomizableTransform, InvertibleTransform, LazyTransform):
    ...
    def __call__(
        ...
        if self._do_transform:
            return rotate90(img, self._rand_k, self.spatial_axes, lazy_evaluation=lazy_)
        else:
            return identity(img, None, None, lazy_evaluation=lazy_)
...
class RandRotate(RandomizableTransform, InvertibleTransform, LazyTransform):
    ...
    def __call__(
        ...
        if self._do_transform:
            ...
            return rotate(img, angles, self.keep_size, mode_, padding_mode_, align_corners_, dtype_, lazy_evaluation=lazy_)
        else:
            return identity(img, None, None, lazy_evaluation=lazy_)
...
class RandFlip(RandomizableTransform, InvertibleTransform, LazyTransform):
    ...
    def __call__(
        ...
        if self._do_transform:
            return flip(img, self.spatial_axis, lazy_evaluation=lazy_)
        else:
            return identity(img, None, None, lazy_evaluation=lazy_)
...
class RandAxisFlip(RandomizableTransform, InvertibleTransform, LazyTransform):
    ...
    def __call__(
        ...
        lazy_ = self.lazy_evaluation if lazy_evaluation is None else lazy_evaluation
        if self._do_transform:
            return flip(img, self._rand_axis, lazy_evaluation=lazy_)
        else:
            return identity(img, None, None, lazy_evaluation=lazy_)
...
class RandZoom(RandomizableTransform, InvertibleTransform, LazyTransform):
    ...
    def __call__(
        ...
        if self._do_transform:
            ...
            return zoom(img, self._rand_zoom, mode_, padding_mode_, align_corners_, self.keep_size, lazy_evaluation=lazy_)
        else:
            return identity(img, None, None, lazy_evaluation=lazy_)
...
class AffineGrid(LazyTransform):
    ...
    def __call__(
        self, spatial_size: Sequence[int] | None = None, grid: torch.Tensor | None = None, lazy_evaluation: bool | None = None
...
class Affine(InvertibleTransform, LazyTransform):
    ...
    def __call__(
        ...
        grid, affine = self.affine_grid(spatial_size=sp_size, lazy_evaluation=lazy_)
...
class RandAffine(RandomizableTransform, InvertibleTransform, LazyTransform):
    ...
    def __init__(
        ...
        self._cached_grid = self._init_identity_cache(lazy_evaluation)
        self.mode = mode
        self.padding_mode: str = padding_mode
    ...
    def _init_identity_cache(self, lazy_evaluation: bool):
        """
    ...
    def get_identity_grid(self, spatial_size: Sequence[int], lazy_evaluation: bool):
        """
    ...
    def __call__(
        ...
        if lazy_:
            if self._do_transform:
                if grid is None:
                    self.rand_affine_grid(sp_size, randomize=randomize, lazy_evaluation=True)
            ...
            if grid is None:
                ...
                if self._do_transform:
                    grid = self.rand_affine_grid(grid=grid, randomize=randomize, lazy_evaluation=lazy_)
            affine = self.rand_affine_grid.get_transformation_matrix()
        return affine_func(  # type: ignore
            img,
            affine,
...
class TransformLike(InvertibleTransform, LazyTransform):
    ...
    def __call__(self, data, reference, lazy_evaluation=None):

        lazy_ = self.lazy_evaluation if lazy_evaluation is None else self.lazy_evaluation
        data = convert_to_tensor(data, track_meta=get_track_meta())

        data_t = transform_like(data=data, reference=reference, lazy_evaluation=lazy_)
