...
def spatial_resample(
    ...
    if src_affine.shape[0] != spatial_rank + 1:
        warnings.warn(
            f"Expected `affine_matrix` of size ({spatial_rank+1},{spatial_rank+1}) for {spatial_rank}D input got ({src_affine.shape[0]},{src_affine.shape[0]}) . The affine matrix will be truncated for {spatial_rank}D input."
        )
