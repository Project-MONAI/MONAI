...
from monai.transforms.io.array import LoadImage, SaveImage, DownloadImage, UploadImage
from monai.transforms.transform import MapTransform
from monai.utils import GridSampleMode, GridSamplePadMode, InterpolateMode, ensure_tuple, ensure_tuple_rep
from monai.utils.enums import PostFix

__all__ = ["LoadImaged", "LoadImageD", "LoadImageDict", "SaveImaged", "SaveImageD", "SaveImageDict", "DownloadImaged", "DownloadImageD", "DownloadImageDict", "UploadImaged", "UploadImageD", "UploadImageDict"]
...
class DownloadImaged(MapTransform):
    """
    Dictionary-based wrapper of :py:class:`monai.transforms.DownloadImage`,
    It can load both image data and metadata. When loading a list of files in one key,
    the arrays will be stacked and a new dimension will be added as the first dimension
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        super().__init__(keys, allow_missing_keys)
        self._loader = DownloadImage(network_downloader, reader, image_only, dtype, ensure_channel_first, *args, **kwargs)
        if not isinstance(meta_key_postfix, str):
            raise TypeError(f"meta_key_postfix must be a str but is {type(meta_key_postfix).__name__}.")
        self.meta_keys = ensure_tuple_rep(None, len(self.keys)) if meta_keys is None else ensure_tuple(meta_keys)
        if len(self.keys) != len(self.meta_keys):
...
class UploadImaged(MapTransform):
    """
    Dictionary-based wrapper of :py:class:`monai.transforms.UploadImage`.
...
DownloadImageD = DownloadImageDict = DownloadImaged
UploadImageD = UploadImageDict = UploadImaged
