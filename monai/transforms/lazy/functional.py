...
def apply_pending(data: torch.Tensor | MetaTensor, pending: list | None = None, overrides: dict | None = None):
    ...
    rank = max(1, cumulative_xform.shape[0] - 1)
    if cumulative_xform.shape[0] < rank + 1:
        cumulative_xform = to_affine_nd(rank, cumulative_xform)
    ...
    for p in pending[1:]:
        ...
        if next_matrix.shape[0] < rank + 1:
            next_matrix = to_affine_nd(rank, next_matrix)
