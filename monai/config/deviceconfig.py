...
def get_gpu_info() -> OrderedDict:
    ...
    for gpu in range(num_gpus):
        ...
        _dict_append(output, f"GPU {gpu} Cached memory (GB)", lambda: round(torch.cuda.memory_reserved(gpu) / 1024 ** 3, 1))
        _dict_append(output, f"GPU {gpu} Allocated memory (GB)", lambda: round(torch.cuda.memory_allocated(gpu) / 1024 ** 3, 1))
