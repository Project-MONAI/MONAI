...
void BilateralFilterCuda(torch::Tensor inputTensor, torch::Tensor outputTensor, float spatialSigma, float colorSigma)
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(inputTensor.type(), "BilateralFilterCudaKernel", ([&] {
        
        // Dispatch kernel. (Partial template function specialisation not supported at present so using this switch instead)
        switch(D)
        {
            case(1): BilateralFilterCudaKernel1D<scalar_t, C><<<dim3(desc.channelStride, desc.batchCount), dim3(1, 1)>>>(inputTensor.data_ptr<scalar_t>(), outputTensor.data_ptr<scalar_t>()); break;
            case(2): BilateralFilterCudaKernel2D<scalar_t, C><<<dim3(desc.channelStride, desc.batchCount), dim3(1, 1)>>>(inputTensor.data_ptr<scalar_t>(), outputTensor.data_ptr<scalar_t>()); break;
            case(3): BilateralFilterCudaKernel3D<scalar_t, C><<<dim3(desc.channelStride, desc.batchCount), dim3(1, 1)>>>(inputTensor.data_ptr<scalar_t>(), outputTensor.data_ptr<scalar_t>()); break;
        }
