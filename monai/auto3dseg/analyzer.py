...
from monai.utils.enums import ImageStatsKeys, LabelStatsKeys, DataStatsKeys
...
class ImageHistogram(Analyzer):
    ...
    def __init__(self, image_key: str, stats_name: str = DataStatsKeys.IMAGE_HISTOGRAM,
                 num_of_bins: list = [100],
                 hist_range: list = [-1000]):
    ...
    def __call__(self, data) -> dict:
        ...
        if len(self.num_of_bins) == 1:
            self.num_of_bins = nr_channels * self.num_of_bins
        if len(self.num_of_bins) != nr_channels:
            raise ValueError(f"There is a mismatch between the number of channels ({nr_channels}) "
                             f"and number histogram bins ({len(self.num_of_bins)}).")
        ...
        for channel in range(nr_channels):
            counts, bin_edges = np.histogram(ndas[channel, ...], bins=self.num_of_bins[channel],
            ...
            if not summ_histogram:
                ...
                for k in range(len(summ_histogram)):
                    ...
                    if np.all(summ_histogram[k]["bin_edges"] != d[DataStatsKeys.IMAGE_HISTOGRAM][k]["bin_edges"]):
                        raise ValueError(f"bin edges are not consistent! {summ_histogram[k]['bin_edges']} vs. {d[DataStatsKeys.IMAGE_HISTOGRAM][k]['bin_edges']}")
