...
class ImageStats(Analyzer):
    ...
    def __call__(self, data):
        ...
        report[ImageStatsKeys.SIZEMM] = [a * b for a, b in zip(report[ImageStatsKeys.SHAPE][0], report[ImageStatsKeys.SPACING])]
...
class LabelStats(Analyzer):
    ...
    def __init__(self, image_key: str, label_key: str, stats_name: str = DataStatsKeys.LABEL_STATS, do_ccp: bool | None = True):
        self.image_key = image_key
        self.label_key = label_key
        self.do_ccp = do_ccp
        ...
        if self.do_ccp:
            report_format[LabelStatsKeys.LABEL][0].update({LabelStatsKeys.LABEL_SHAPE: None, LabelStatsKeys.LABEL_NCOMP: None})
...
class LabelStatsSumm(Analyzer):
    ...
    def __init__(self, stats_name: str = DataStatsKeys.LABEL_STATS, average: bool | None = True, do_ccp: bool | None = True):
        self.summary_average = average
        self.do_ccp = do_ccp
    ...
    def __call__(self, data: list[dict]) -> dict:
        ...
        for label_id in unique_label:
            ...
            stats[intst_str] = self.ops[label_str][0][intst_str].evaluate(intst_dict, dim=None if self.summary_average else 0)
...
class ImageHistogram(Analyzer):
    ...
    def __init__(
        ...
        # set defaults
        self.hist_bins: list[int] = [100] if hist_bins is None else hist_bins if isinstance(hist_bins, list) else [hist_bins]
