...
def save_net_with_metadata(
    ...
    """
    Save the JIT object (script or trace produced object) `model_obj` to the given file or stream with metadata
    ...
    Args:
        model_obj: object to save, should be generated by `script` or `trace`.
        filepath: filename or file-like stream object, if filename has no extension it becomes `.ts`.
    ...
    if isinstance(filepath, str):
        ...
        if append_timestamp:
            filepath = now.strftime(f"{filename_no_ext}_%Y%m%d%H%M%S{ext}")
        else:
