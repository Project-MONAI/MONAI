...
class BaseWSIReader(ImageReader):
    ...
    def _compute_mpp_tolerances(self, closest_lvl, mpp_list, mpp, atol, rtol) -> tuple[bool, bool]:
        """
...
class CuCIMWSIReader(BaseWSIReader):
    ...
    def get_wsi_at_mpp(
        ...
        if within_tolerance:
            ...
            closest_lvl_wsi = wsi.read_region(
                (0, 0),
                level=closest_lvl,
                size=wsi.resolutions["level_dimensions"][closest_lvl],
        ...
        wsi_arr = np.moveaxis(wsi_arr, -1, self.channel_dim)

        # Apply mode (RGB / RGBA)
        if self.mode == "RGB":
            if wsi_arr.shape[self.channel_dim] not in [3, 4]:
                raise ValueError(
                    f"The image is expected to have three or four color channels in '{self.mode}' mode but has "
                    f"{wsi_arr.shape[self.channel_dim]}. "
                )
            wsi_arr = np.take(wsi_arr, [0, 1, 2], self.channel_dim)
    ...
    def _resize_to_mpp_res(self, wsi, closest_lvl, mpp_list, user_mpp: tuple):
        ...
        wsi_arr = cp.array(
            wsi.read_region((0, 0), level=closest_lvl, size=closest_lvl_dim, num_workers=self.num_workers)
        )
        closest_lvl_wsi = cucim_resize(
            wsi_arr, (target_res_x, target_res_y), order=1, preserve_range=True, anti_aliasing=False
        ).astype(cp.uint8)
...
class OpenSlideWSIReader(BaseWSIReader):
    ...
    def get_wsi_at_mpp(
        ...
        if within_tolerance:
            # If the image at the desired mpp resolution is within tolerances, return the image at closest_level.
            closest_lvl_wsi = wsi.read_region((0, 0), level=closest_lvl, size=wsi.level_dimensions[closest_lvl])
        ...
        # Convert to specified mode (e.g., RGBA to RGB)
        closest_lvl_wsi = closest_lvl_wsi.convert(self.mode)

        wsi_arr = np.array(closest_lvl_wsi)

        # Ensure channel dimension exists for grayscale (defensive: PIL.convert
        # to "L" produces a 2-D array without a trailing channel axis).
        if wsi_arr.ndim < 3:
            wsi_arr = wsi_arr[..., None]
...
class TiffFileWSIReader(BaseWSIReader):
    ...
    def get_mpp(self, wsi, level: int) -> tuple[float, float]:
        ...
        if (
            ...
            if xres[0] and yres[0]:
                ...
                raise ValueError(
                    "The `XResolution` and/or `YResolution` property of the image is zero, "
    ...
    def _resize_to_mpp_res(self, wsi, closest_lvl, mpp_list, user_mpp: tuple):
        ...
        # `_compute_mpp_target_res` expects (W, H) order, matching the OpenSlide /
        # CuCIM convention; `self.get_size` returns (H, W), so swap before passing.
