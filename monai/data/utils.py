...
def get_random_patch(
    dims, patch_size, rand_state=None,
):
    """
    ...
    min_corner = tuple(
        rand_int(0, ms - ps,) if ms > ps else 0 for ms, ps in zip(dims, patch_size,)
    )
...
def iter_patch_slices(
    ...
    for position in product(
        *ranges[::-1]
    ):  # reverse ranges order to iterate in index order
        yield tuple(slice(s, s + p,) for s, p in zip(position[::-1], patch_size,))


def dense_patch_slices(
    image_size, patch_size, scan_interval,
):
    """
...
def iter_patch(
    arr, patch_size, start_pos=(), copy_back=True, pad_mode="wrap", **pad_opts,
):
    """
...
def compute_shape_offset(
    spatial_shape, in_affine, out_affine,
):
    """
    ...
    corners = np.asarray(np.meshgrid(*in_coords, indexing="ij",)).reshape(
        (len(shape), -1,)
    )
...
def to_affine_nd(
    ...
    new_affine[:d, :d,] = affine[
        :d, :d,
    ]
    if d > 1:
        new_affine[:d, -1,] = affine[
            :d, -1,
        ]
    return new_affine
