...
def resolve_name(name,):
    ...
    if obj is None:
        ...
        if len(mods) > 0:  # found modules with this declaration or import
            if (
                len(mods) > 1
            ):  # found multiple modules, need to determine if ambiguous or just multiple imports
                foundmods = {
                    inspect.getmodule(getattr(m, name,)) for m in mods
                }  # resolve imports
                ...
                if len(foundmods) > 1:  # found multiple declarations with the same name
                    ...
                    msg = (
                        "Multiple modules (%r) with declaration name %r found, resolution is ambiguous"
                        % (modnames, name,)
                    )
