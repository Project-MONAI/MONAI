...
from .utils import sliding_window_inference_multioutput

__all__ = ["SlidingWindowMultiOutputInferer"]
...
class SlidingWindowMultiOutputInferer(Inferer):
    ...
    def __init__(
        ...
        try:
            if isinstance(roi_size, Sequence) and min(roi_size) > 0:
                if device is None:
                    device = "cpu"
                self.importance_map_roi_size = compute_importance_map(
    ...
    def __call__(
        ...
        """
        return sliding_window_inference_multioutput(
            inputs,
            self.roi_size,
            self.sw_batch_size,
            network,
