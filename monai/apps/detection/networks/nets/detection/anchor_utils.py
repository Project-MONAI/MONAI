...
class AnchorGenerator(nn.Module):
    ...
    def generate_anchors(
        ...
        if self.spatial_dims != len(aspect_ratios.shape) - 1:
            ValueError(
                f"In {self.spatial_dims}-D image, aspect_ratios for each level should be {len(aspect_ratios.shape)-1}-D. But got aspect_ratios with shape {aspect_ratios.shape}."
            )

        if (self.spatial_dims == 3) and (aspect_ratios.shape[1] != 2):
            ValueError(
                f"In {self.spatial_dims}-D image, aspect_ratios for each level should has shape (_,2). But got aspect_ratios with shape {aspect_ratios.shape}."
            )
