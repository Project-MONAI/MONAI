...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    output_channels = parser.get_parsed_content("output_channels")
    ...
    if softmax:
        post_pred = transforms.Compose(
            [transforms.EnsureType(), transforms.AsDiscrete(argmax=True, to_onehot=output_channels)]
        )
        post_label = transforms.Compose([transforms.EnsureType(), transforms.AsDiscrete(to_onehot=output_channels)])
    else:
        post_pred = transforms.Compose(
