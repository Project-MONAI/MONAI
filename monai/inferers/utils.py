...
__all__ = ["sliding_window_inference", "sliding_window_inference_multioutput"]
...
def sliding_window_inference_multioutput(
    ...
    **kwargs: Any,
    ...
    The outputs of the `preditor` should be a list of Tensors or a dict with Tensor values.
    Each output in the list/dict is allowed to have different resolutions with the input.
    ...
    Args:
        ...
        sw_batch_size: the batch size to run window slices.
        predictor: given input tensor `patch_data` in shape NCHW[D], `predictor(patch_data)`
            should return a list prediction with the same batch_size, i.e. NM'H'W'[D'];
    ...
    for slice_g in range(0, total_slices, sw_batch_size):
        ...
        window_data = torch.cat([inputs[win_slice] for win_slice in unravel_slice]).to(sw_device)
        seg_prob_list = predictor(window_data, *args, **kwargs)  # batched patch segmentation
        if isinstance(seg_prob_list, dict):
