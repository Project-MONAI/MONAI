...
def main():
    ...
    check_loader = DataLoader(
        check_ds, batch_size=2, num_workers=4, pin_memory=torch.cuda.is_available(),
    )
    check_data = monai.utils.misc.first(check_loader)
    print(
        check_data["img"].shape, check_data["label"],
    )
    ...
    checkpoint_handler = ModelCheckpoint(
        "./runs/", "net", n_saved=10, require_empty=False,
    )
    ...
    evaluator = create_supervised_evaluator(
        net, val_metrics, device, True, prepare_batch=prepare_batch,
    )
