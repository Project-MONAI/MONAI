...
def main():
    ...
    for i in range(5):
        ...
        nib.save(
            n, os.path.join(tempdir, "im%i.nii.gz" % i,),
        )

        n = nib.Nifti1Image(seg, np.eye(4),)
        nib.save(
            n, os.path.join(tempdir, "seg%i.nii.gz" % i,),
        )
    ...
    ds = NiftiDataset(
        images, segs, transform=imtrans, seg_transform=segtrans, image_only=False,
    )
    ...
    MeanDice(add_sigmoid=True, to_onehot_y=False,).attach(
        evaluator, "Mean_Dice",
    )
    ...
    ckpt_saver = CheckpointLoader(
        load_path="./runs/net_checkpoint_50.pth", load_dict={"net": net},
    )
