...
def bilateral_phl(input_tensor, spatial_sigma, color_sigma):
    ...
    input_tensor = torch.flatten(input_tensor, start_dim=2)
    ...
    normalisation_mask_tensor = phl.apply(feature_tensor, torch.ones_like(input_tensor))
    filtered_tensor = phl.apply(feature_tensor, input_tensor) / normalisation_mask_tensor

    # reshaping output
    filtered_tensor = filtered_tensor.reshape(input_tensor_shape)
...
plt.title('pytorch phl: {}sec'.format('%.2f' % pyt_phl_time))
plt.imshow(pyt_phl_result)
plt.subplot(223).axis('off')
plt.title('c++ simple: {}sec'.format('%.2f' % cpp_std_time))
plt.imshow(cpp_std_result)
plt.subplot(224).axis('off')
plt.title('c++ phl: {}sec'.format('%.2f' % cpp_phl_time))
